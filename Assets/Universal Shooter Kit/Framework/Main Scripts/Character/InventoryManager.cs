    ...
    public class InventoryManager : MonoBehaviour
    {
        ...
        public void ClearAllWeapons()
        {
            ...
            for (var i = 0; i < 8; i++)
            {
                ...
                var slotsToRemove = slots[i].weaponSlotInGame.Where(weaponSlot => !weaponSlot.fistAttack).ToList();

                foreach (var slotToRemove in slotsToRemove)
                {
                    slots[i].weaponSlotInGame.Remove(slotToRemove);
                }
...
#endif
#if USK_ADVANCED_MULTIPLAYER
                || Controller.amGameData.weaponsUsageMethod == AMHelper.WeaponsMethodOfUse.Inventory
#endif
            )
            ...
            else
            {
                if (Controller.amGameData)
                {
                    for (var i = 0; i < Controller.amGameData.weaponSlots.Count; i++)
                    {
                        ...
                        {
                            if (Controller.amGameData.weaponsUsageMethod == AMHelper.WeaponsMethodOfUse.InGameShop)
                            {
                            ...
                            else if(Controller.amGameData.weaponsUsageMethod == AMHelper.WeaponsMethodOfUse.Specific)
                            {
                                WeaponsHelper.InstantiateWeapon(weaponSlot.weapon.gameObject, weaponSlot.slot, this, Controller);
                            }
                        }
