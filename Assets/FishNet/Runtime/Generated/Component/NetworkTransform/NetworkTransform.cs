...
#pragma warning restore CS0414 //Not in use.
        ...
        private void SendToClients(byte lodIndex)
        {
            ...
            if (clientAuthoritativeWithOwner && !base.Owner.IsLocalClient)
            {
                if (_receivedClientData.HasData[lodIndex])
                {
                    ...
                    ObserversUpdateClientAuthoritativeTransform(_receivedClientData.Writer.GetArraySegment(), _receivedClientData.Channel);
        ...
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void SetCalculatedRates(uint lastTick, byte lodIndex, RateData prevRd, TransformData prevTd, GoalData nextGd, ChangedFull changedFull, bool hasChanged, Channel channel)
        {
            ...
            if (ChangedFullContains(changedFull, ChangedFull.Position))
            {
                ...
                bool teleport = false;
                /* Teleport if the distance is extremely small.
                 * This fixes cases where the distance is so insignificant
                 * that a move rate cannot be calculated. */
                teleport |= (distance < 0.0001f);
                //If distance teleports assume rest do.
                if (!teleport && _enableTeleport)
                {
        ...
        private void ServerUpdateTransform(ArraySegment<byte> data, Channel channel)
        {
            ...
            _receivedClientData.Update(data, channel, true);
        ...
        private void DataReceived(ArraySegment<byte> data, Channel channel, bool asServer)
        {
            ...
            //Otherwise use timed.
                ...
                SetCalculatedRates(prevTd.Tick, lodIndex, prevRd, prevTd, nextGd, changedFull, hasChanged, channel);
