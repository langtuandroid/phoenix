...
namespace GercStudio.USK.Scripts
{
    public class PackageManager : EditorWindow
    {
        ...
        void Init()
        {
            ...
            results = AssetDatabase.FindAssets("ComponentNUIPropertyDrawer t:MonoScript");
            if (results.Length > 0)
            {
                asset = results[0];
                NWHPath = AssetDatabase.GUIDToAssetPath(asset);
                var replace = NWHPath.Replace("Scripts/VehicleController/ComponentNUIPropertyDrawer.cs", "");
                NWHPath = replace;
            }
